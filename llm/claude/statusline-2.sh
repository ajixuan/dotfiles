...
pr_color() { if [ "$use_color" -eq 1 ]; then printf '\033[38;5;75m'; fi; }  # soft blue
rst() { if [ "$use_color" -eq 1 ]; then printf '\033[0m'; fi; }
...
if git rev-parse --git-dir >/dev/null 2>&1; then
  ...
  pr_info=$(gh pr list --head "$git_branch" --json number,title --limit 1 | jq -r '.[0].number // empty')
fi
...
if command -v bunx ccusage@latest >/dev/null 2>&1 && [ "$HAS_JQ" -eq 1 ]; then
  blocks_output=""

  # Try ccusage with timeout
  if command -v timeout >/dev/null 2>&1; then
    blocks_output=$(timeout 10s bunx ccusage@latest blocks --json 2>/dev/null)
  elif command -v gtimeout >/dev/null 2>&1; then
    blocks_output=$(gtimeout 10s bunx ccusage@latest blocks --json 2>/dev/null)
  else
    blocks_output=$(bunx ccusage blocks --json 2>/dev/null)
  fi
...
if [ -n "$pr_info" ]; then
  printf '  🔀 %s#%s%s' "$(pr_color)" "$pr_info" "$(rst)"
fi
...
# TODO: Decide wat you want this to be
if [ -n "$usage_text" ]; then
  if [ -n "$line2" ]; then
    line2="$line2  ⌛ $(session_color)${usage_text}$(rst) $(session_color)[${session_bar}]$(rst)"
  else
    line2="⌛ $(session_color)${usage_text}$(rst) $(session_color)[${session_bar}]$(rst)"
  fi
fi
